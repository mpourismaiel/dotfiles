...
local function update_status(self)
  ...
  text = text == "ir" and "fa" or text == "us" and "en" or text

  if self.markup_fn ~= nil then
    self.widget:set_markup(self.markup_fn(string.upper(text)))
  else
    self.widget:set_markup(
      markup(
        "#ffffff99",
        (self.icon or "") .. markup.font("FiraCode Bold 12", string.upper(text))
      )
    )
  end
end
...
function keyboardlayout.new(args)
  ...
  -- Mouse bindings
  self:buttons(gtable.join(button({}, 1, self.relaunch_layout)))

  return self
end

function keyboardlayout.mt:__call(...)
  return keyboardlayout.new(...)
end
