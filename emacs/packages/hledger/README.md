...
- Editing uses `ledger-mode` (installed in `mp-langs.el`) with hledger as the
  binary — font-lock, `TAB` alignment, account completion and the evil
  `[[` / `]]` motions all work.
- Two stock `ledger-mode` commands are ledger-CLI-only and are avoided:
  ...
  `ledger-reconcile`.
- On-the-fly checking comes from `flycheck-hledger` (declared here via
  `use-package`); the stock `ledger` flycheck checker is disabled here.

## Loading

`mp-tools.el` loads it with `(mp/require-package "hledger")`; the `SPC o l`
finance bindings live in `mp-keys.el`.
