...
- **Health** — LSP status (reports the active backend: `lsp-bridge` or
  `eglot`); **Diagnostics** count `NE MW` (click → open the error/warning
  list; counted via flycheck/flymake — eglot reports through flymake).
  **Hidden entirely when no LSP server is running.**
...
- `SPC t W` (bound in `lisp/mp-keys.el`, or `M-x mp/workspace-hud-toggle`) —
  toggle.
