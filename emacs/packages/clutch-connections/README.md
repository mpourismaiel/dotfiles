...
Leader binding: `mp-keys.el` owns all `SPC` leader bindings — bind
`mp/clutch-connections` there (e.g. under `SPC o`), not in this package.
...
- **`gpg`** — EasyPG-encrypted `connections.json.gpg` in the config root
  (`mp/emacs-dir`). Encrypted
...
`mp-tools.el` loads it in place with `(mp/require-package "clutch-connections")`
(clutch itself is declared there via `use-package` / elpaca).
