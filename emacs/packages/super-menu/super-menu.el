...
;;
;; Ported from the Doom config to vanilla Emacs (perspective.el instead of
;; Doom workspaces, stock file commands instead of doom/* helpers).
;;; Code:
...
(defun mp/workspace-buffers ()
  "Return buffers belonging to the current perspective."
  ;; Doom's `+workspace-buffer-list' -> perspective.el's `persp-current-buffers'.
  (if (fboundp 'persp-current-buffers)
      (seq-filter #'buffer-live-p (persp-current-buffers))
    (buffer-list)))

(defun mp/current-workspace-buffer-p (buf)
  "Return non-nil if BUF belongs to the current perspective."
...
(defun mp/super-menu--interesting-buffer-p (buf)
  "Return non-nil if BUF is worth listing at all (a \"real\" buffer).
Replacement for Doom's real-buffer notion: BUF visits a file, is a
Ghostel/agent-shell terminal, or is an ordinary user-facing buffer.
Tool buffers with innocent-looking names — magit status (\"magit: proj\"),
dired/dirvish directory buffers (\"proj\"), dirvish previews
(\"PREVIEW :: …\") — are excluded even though they aren't earmuffed."
       ...
       (or (buffer-file-name (or (buffer-base-buffer buf) buf))
           ...
           (let ((name (buffer-name buf)))
             (and (not (string-prefix-p " " name))
                  ...
                  (not (string-match-p "\\`PREVIEW\\b" name))
                  (not (with-current-buffer buf
                         (derived-mode-p 'magit-section-mode 'magit-mode
                                         'magit-process-mode 'dired-mode
                                         'image-mode 'special-mode
                                         'compilation-mode))))))))
...
GitHub-PR buffers always show in the primary [B]uffer list."
  (and (mp/super-menu--interesting-buffer-p buf)
...
(defvar mp/consult-source-live-project-file
  `(:name "[F]ile"
    ...
    :enabled ,(lambda ()
                (or (and (fboundp 'mp/current-workspace-project-root)
                         (mp/current-workspace-project-root))
                    (and (bound-and-true-p projectile-mode)
                         (projectile-project-p))))
    :items ,(lambda ()
...
(defun mp/super-menu--delete-this-file ()
  "Delete the file the current buffer visits, then kill the buffer.
Stock replacement for Doom's `doom/delete-this-file'."
  (interactive)
...
(defun mp/super-menu-run-action (action)
  ...
  (pcase action
    ;; `doom/move-this-file' -> stock `rename-visited-file' (Emacs 29+).
    ('rename   (mp/super-menu--with-origin-buffer #'rename-visited-file))
    ;; `doom/delete-this-file' -> local helper.
    ('delete   (mp/super-menu--with-origin-buffer #'mp/super-menu--delete-this-file))
...
;; Doom's `(map! :leader "SPC" #'mp/super-menu)' removed: mp-keys.el already
;; binds SPC SPC to `mp/super-menu'.
