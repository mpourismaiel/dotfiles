...
QtObject {
    ...
    readonly property string confirmation: "random"
}
#+end_src
...
Row {
    ...
    IconImage {
        ...
        source: Quickshell.iconPath(root.notifs.dnd ? "notifications-disabled" : "notifications", root.notifs.dnd ? "notification-disabled" : "notification")
...
KWIN_JS = (
    # each taskbar window is reported as { id, seq, cls, dfile }: seq is a stable
    # open-order index (assigned the first time a window is seen, so newly opened
    # windows sort after older ones), cls is the app's resourceClass and dfile its
    # desktopFileName — both used by the pill to group by app and resolve a proper
    # icon (the KRunner icon name alone often misses).
    "var seqN=0, seq={};\n"
...
// The hover highlight is NOT drawn here (only the open-menu selection): each icon
// reports itself via itemEntered (on enter only) so the shared hover square jumps to it.
import QtQuick
import Quickshell
import Quickshell.Widgets
import Quickshell.Services.Pipewire
...
Row {
    ...
    spacing: root.theme.iconSpacing
    Repeater {
        model: [
        ...
        delegate: Rectangle {
            id: statusItem
            required property var modelData
            width: root.theme.iconCell; height: root.theme.iconCell; radius: root.theme.radiusSmall
            // only the open-menu selection is drawn here; hover is the shared square
            color: (root.open && root.menu === modelData.menu) ? root.theme.accentDim : "transparent"
            Behavior on color { ColorAnimation { duration: root.theme.animFast } }
            IconImage {
                anchors.centerIn: parent
                implicitSize: 18
...
Rectangle {
    id: root
    required property var theme
    required property var group          // { icon, entry, wins: [{ id, title }] }
...
2. *hovered* (=dash=, 520×116): two balanced rows — Row 1 date/time with the
   virtual-desktop dots stacked beneath it (left) | grouped app icons (right);
   Row 2 a launcher button (left) | system tray + status icons
...
Variants {
    ...
    PanelWindow {
        ...
        color: "transparent"
        mask: (win.open || win.launcher || win.ctxGroup !== null || win.confirmActive) ? fullRegion : (win.fsHide ? emptyRegion : pillRegion)
        ...
        Item {
            ...
            height: win.showBurst ? 28 : (win.open || win.launcher) ? win.openHeight : (win.notifMorph ? morphStack.implicitHeight : (win.dash ? 116 : 28))
            ...
            Rectangle {
                ...
                readonly property var target: win.hoverItem ? win.hoverItem : launcherIcon
                ...
                radius: theme.radiusSmall
                color: theme.bgHover
                opacity: (win.dash && !win.notifMorph && target) ? 1 : 0
                visible: opacity > 0
                x: pos.x - pad
                y: pos.y - pad
            ...
            Item {
                ...
                // open the app launcher (the virtual-desktop dots now live under
                // the datetime in Row 1, so this stays alone on the left). This is
                // also the shared hover square's resting spot, so it draws no
                // highlight of its own — the square rests here by default.
                Rectangle {
                    id: launcherIcon
                            ...
                            MouseArea {
                                ...
                                onClicked: (mouse) => {
                                    if (mouse.button === Qt.LeftButton)
                                        ...
                                        trayItem.modelData.display(win, width / 2, height);
                                }
                            }
            ...
            Item {
                ...
                y: 104
                width: pill.width - theme.pad * 2
                height: pill.height - 104 - theme.pad
            ...
            Item {
                ...
                Launcher {
                    ...
                    // bottom must line up with the clipping container above
                    // (which ends at pill.height - 104 - pad); the y:14 offset +
                    // this 104 leave the power / toggle rows fully inside the
                    // clip instead of half-cut at the panel's bottom edge.
                    height: win.openHeight - 104 - theme.pad - 14
                    theme: theme
                    apps: root.appList
                    settings: settings
                    notifs: notifs
        ...
        Rectangle {
            ...
            Component {
                ...
                Item {
                    ...
                    Rectangle {
                        ...
                        gradient: Gradient {
                            GradientStop { position: 0.0; color: "#17131a" }
                            GradientStop { position: 0.55; color: "#0d0b0e" }
                            GradientStop { position: 1.0; color: "#090809" }
                        }
                    }
                    ...
                    Column {
                        ...
                        spacing: 0
                        opacity: 0
                        transform: Translate { id: hushTr; y: 14 }

                        NumberAnimation { target: hushCol; property: "opacity"; from: 0; to: 1; duration: 700; easing.type: Easing.OutCubic; running: true }
                        NumberAnimation { target: hushTr; property: "y"; from: 14; to: 0; duration: 700; easing.type: Easing.OutCubic; running: true }
                        ...
                        Row {
                            ...
                            Column {
                                ...
                                Rectangle { width: parent.width; height: 2; color: "#cf4a2c" }
                            }
            ...
            Component {
                ...
                Item {
                    ...
                    Rectangle { anchors.fill: parent; color: "#cf4a2c" }
                    ...
                    Column {
                        ...
                        spacing: 24
                        opacity: 0
                        transform: Translate { id: blazeTr; y: 14 }

                        NumberAnimation { target: blazeCol; property: "opacity"; from: 0; to: 1; duration: 700; easing.type: Easing.OutCubic; running: true }
                        NumberAnimation { target: blazeTr; property: "y"; from: 14; to: 0; duration: 700; easing.type: Easing.OutCubic; running: true }
                    ...
                    Row {
                        ...
                        Rectangle {
                            ...
                            Rectangle { anchors.left: parent.left; width: 2; height: parent.height; color: "#a5361f" }
            ...
            Component {
                ...
                Item {
                    ...
                    Rectangle { anchors.fill: parent; color: "#0d0b0e" }
                    ...
                    Column {
                        ...
                        spacing: 26
                        opacity: 0
                        transform: Translate { id: ledgerTr; y: 14 }

                        NumberAnimation { target: ledgerCol; property: "opacity"; from: 0; to: 1; duration: 700; easing.type: Easing.OutCubic; running: true }
                        NumberAnimation { target: ledgerTr; property: "y"; from: 14; to: 0; duration: 700; easing.type: Easing.OutCubic; running: true }
                        ...
                        Item {
                            ...
                            Rectangle {
                                ...
                                SequentialAnimation on opacity {
                                    ...
                                    NumberAnimation { from: 0.55; to: 0.9; duration: 1200; easing.type: Easing.InOutSine }
                                    NumberAnimation { from: 0.9; to: 0.55; duration: 1200; easing.type: Easing.InOutSine }
                                }
                            }
