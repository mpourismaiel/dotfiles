...
Item {
    ...
    signal appMenuRequested(var app, bool fav, real x, real y)
                ...
                MouseArea {
                    ...
                    onClicked: (mouse) => {
                        if (mouse.button === Qt.RightButton) {
                            ...
                            root.appMenuRequested(appRow.modelData, root.isFav(appRow.modelData), p.x, p.y);
                        } else {
                ...
                MouseArea {
                    ...
                    onClicked: (mouse) => {
                        if (mouse.button === Qt.RightButton) {
                            ...
                            if (e) {
                                const p = mapToItem(root, mouse.x, mouse.y);
                                root.appMenuRequested(e, root.isFav(e), p.x, p.y);
                            }
                            return ;
                        }
            ...
            model: DelegateModel {
                ...
                delegate: MouseArea {
                    ...
                    onClicked: (mouse) => {
                        if (mouse.button === Qt.RightButton) {
                            const p = mapToItem(root, mouse.x, mouse.y);
                            root.appMenuRequested(tile.modelData, root.isFav(tile.modelData), p.x, p.y);
                        } else {
...
separator + an Add/Remove favourite toggle) but *without* the window actions.
...
overlay and calls =openFor=. Favourites are optional and driven by the host
(=showFavourite= / =isFavourite= in, =favouriteToggled(entry)= out) so the component
stays independent of the launcher's settings; running an action or toggling the
favourite closes the menu.

Signals: =launched()= (an action ran), =favouriteToggled(entry)= (the favourite row
was clicked), =dismissed()= (backdrop click / any action closed it). =openFor(entry,
x, y, fav, showFav)= shows it; =close()= hides it. A reusable =CtxRow= draws each row.
*** Code
...
Item {
    ...
    function openFor(e, x, y, fav, showFav) {
        entry = e;
        isFavourite = fav === undefined ? false : fav;
        showFavourite = showFav === undefined ? false : showFav;
        px = x;
        py = y;
    }
    ...
    Rectangle {
        ...
        Column {
            ...
            // separator before the favourite toggle (only when actions sit above it)
            Item {
                visible: root.showFavourite && root.entryActions.length > 0
...
Variants {
    ...
    PanelWindow {
        ...
        Item {
            ...
            Item {
                ...
                Launcher {
                    ...
                    onAppMenuRequested: (app, fav, x, y) => {
                        const p = launcherItem.mapToItem(launcherMenu, x, y);
                        launcherMenu.openFor(app, p.x, p.y, fav, true);
                    }
        ...
        AppMenu {
            ...
            z: 1000
            theme: theme
            onFavouriteToggled: (e) => launcherItem.toggleFav(e)
            onLaunched: win.launcher = false
        }
