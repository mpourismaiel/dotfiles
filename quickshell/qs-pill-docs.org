...
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/MenuKbNav.qml :mkdirp yes
pragma ComponentBehavior: Bound
...
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/SearchField.qml :mkdirp yes
pragma ComponentBehavior: Bound
...
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/PolishConfig.qml :mkdirp yes
pragma ComponentBehavior: Bound
...
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/CalendarEvents.qml :mkdirp yes
pragma ComponentBehavior: Bound
...
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/AccountsState.qml :mkdirp yes
pragma ComponentBehavior: Bound
...
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/AccountsMenu.qml :mkdirp yes
pragma ComponentBehavior: Bound
...
- *Nag* — from 20:00, if today has no finance entry and the (persisted)
  ...
  without a restart.
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/FinanceState.qml :mkdirp yes
pragma ComponentBehavior: Bound
...
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/FinanceMenu.qml :mkdirp yes
pragma ComponentBehavior: Bound
...
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/Memos.qml :mkdirp yes
pragma ComponentBehavior: Bound
...
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/VoiceSetup.qml :mkdirp yes
pragma ComponentBehavior: Bound
...
*** Code
#+begin_src python :tangle ~/.config/quickshell/pill/gcalbridge.py :mkdirp yes
#!/usr/bin/env python3
...
*** Code
#+begin_src python :tangle ~/.config/quickshell/pill/hledgerbridge.py :mkdirp yes
#!/usr/bin/env python3
...
- *Prompt files are user-owned*: =~/.config/quickshell/transcribe/whisper-prompt.md=
  ...
  first run; they live outside the tangle dir so re-tangling never clobbers edits.
...
*** Code
#+begin_src python :tangle ~/.config/quickshell/pill/voicebridge.py :mkdirp yes
#!/usr/bin/env python3
...
*** Code
#+begin_src xml :tangle ~/.config/quickshell/pill/transcribe.svg :mkdirp yes
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="48" height="48">
  <rect width="48" height="48" rx="12" fill="#221d16"/>
  <rect x="19" y="9" width="10" height="19" rx="5" fill="#cf4636"/>
  <path d="M13 24a11 11 0 0 0 22 0" fill="none" stroke="#efe9dd" stroke-width="3" stroke-linecap="round"/>
  <line x1="24" y1="35" x2="24" y2="40" stroke="#efe9dd" stroke-width="3" stroke-linecap="round"/>
  <path d="M37.5 5.5l1.1 2.9 2.9 1.1-2.9 1.1-1.1 2.9-1.1-2.9-2.9-1.1 2.9-1.1z" fill="#d9a441"/>
</svg>
#+end_src
...
*** Code
#+begin_src conf :tangle ~/.config/quickshell/pill/source-rnnoise.conf :mkdirp yes
# source-rnnoise.conf — RNNoise mic filter for the pill's voice recording.
# Run as: pipewire -c ~/.config/quickshell/pill/source-rnnoise.conf
# (spawned by the pill only while recording with noise suppression enabled)
context.properties = {
    log.level = 0
}
context.spa-libs = {
...
context.modules = [
    ...
    { name = libpipewire-module-filter-chain
        args = {
            node.description = "RNNoise (pill transcribe)"
            media.name       = "RNNoise (pill transcribe)"
            filter.graph = {
                nodes = [
                    {
                        type   = ladspa
...
lives *outside* the tangle: the whisper venv, the KDE global shortcuts and the
API key. Run it once after tangling:
...
*** Code
#+begin_src sh :tangle ~/.config/quickshell/pill/setup-transcribe.sh :mkdirp yes :tangle-mode (identity #o755)
#!/usr/bin/env bash
...
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/VoiceRecorderPill.qml :mkdirp yes
pragma ComponentBehavior: Bound
...
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/VoiceMemoMenu.qml :mkdirp yes
pragma ComponentBehavior: Bound
