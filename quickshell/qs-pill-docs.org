...
| Action bursts (desktop switch / layout change)     | ✅ built       | 1s transient takeover of the resting pill (=root.burstDesk=/=burstLang=): desktop dot slides old→new; layout shows =US → FA=; both at once → two pills side by side. Layout via KWin DBus =org.kde.keyboard /Layouts=; coexists with notifications (deck floats below) |
...
4. *launcher* (=launcher=, 520×470): the app launcher takes the menu pane's place,
   *also below the two rows* (same divider + offset), so it matches the menus (see
   ...
   (=view-grid-symbolic=), by clicking the hovered pill body, or by the *Meta+D*
   global shortcut.
...
=registerShortcut("PillLauncher", …, "Meta+D", …)= calls our =org.kde.pill=
...
occasional =\{event:"launcher", screen\}= line for the Meta+D shortcut), which a
...
- *window list*: the KRunner "windows" runner =org.kde.KWin /WindowsRunner=,
  ...
  (the runner returns each window twice). =Run(id, "")= focuses one. While a window
  is *being activated* the runner briefly lists an internal placeholder (icon
  =kwin=, no title); we drop =icon == "kwin"= so it doesn't flash a phantom tile.
...
- *launcher shortcut*: the same script =registerShortcut=s *Meta+D*, whose callback
  calls =org.kde.pill Launcher(activeWindow.output.name)= → the =\{event:"launcher"\}=
  line above (see [[*Global shortcut (Meta+D)][§ Global shortcut]]).
...
=burstDesk= / =burstLang= / =burstPhase=: =startBurst()= runs a single =Timer=
through three phases — *settle 100ms → animate 300ms → settle 600ms* — clearing
both flags when done. The burst surface is *one mini-pill per active kind*, each
carrying the same surface as the *hovered* pill (=theme.bg= + border); the outer
pill goes transparent so the gap between pills shows the backdrop. *Both* kinds can
be live at once (a desktop switch and a layout change within the same window):
then two pills sit *side by side* and *pop apart* on entry — a quick =Translate=
from centre to their slots (=OutBack=, 180ms). A burst only takes over the
*resting* pill (=showBurst=): it yields to hover / open / launcher / a right-click
group (those already show richer UI), and it overrides =notifMorph= so an active
notification deck *floats below* the burst pill instead of being replaced (the
same below-pill =Loader= used for an opened pill) — that is how notifications slide
down to reveal the change. Even under a focused fullscreen window the burst still
*draws* (its input region stays empty, so it never intercepts clicks). Two events
fire bursts today, each guarding against its *initial* value on startup (QML change
handlers carry no old value, so =root= keeps the previous desktop id / layout index
to animate *from*):
- *Virtual-desktop switch* (=onCurrentDesktopChanged=) — renders the same dot row
  ...
  the previous desktop's slot to the new one across the 300ms animate phase
  ...
  dot that slides to =deskActiveIdx= (=Behavior on x=, 300ms =OutCubic=), so
...
ShellRoot {
    ...
    Variants {
        ...
        PanelWindow {
            ...
            Item {
                ...
                anchors.horizontalCenter: parent.horizontalCenter
                y: 6
                width: win.showBurst ? (burstLoader.item ? burstLoader.item.implicitWidth : 96)
                ...
                Rectangle {
                    ...
                    gradient: (win.dash || win.notifMorph || win.showBurst) ? null : restGrad
    ...
    // One mini-pill per active kind, laid out side by side with a gap; each carries
    // the hovered-state surface (theme.bg + border). When *both* kinds are live the
    // two pills pop apart on entry (a quick Translate from centre to their slots).
        ...
        Row {
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    radius: 12
                    color: theme.bg                  // same surface as the hovered pill
                    border.color: theme.border
                    border.width: 1
                    // pop apart: only when two pills are shown, slide from centre out
                    transform: Translate {
                        NumberAnimation on x {
                            running: root.burstKinds.length === 2
                    ...
                    Item {
                        id: cellBody
                        anchors.centerIn: parent
                        implicitWidth: cell.isLang ? langRow.implicitWidth : dotRow.width
                        implicitHeight: 28
