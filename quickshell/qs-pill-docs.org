...
Row {
    ...
    MSym {
        ...
        color: root.notifs.dnd ? root.theme.accent
             : (dndMa.containsMouse ? root.theme.text : root.theme.faint)
...
Row {
    ...
    // live glyphs: volume follows Pipewire mute; battery follows UPower state.
    readonly property bool muted: Pipewire.defaultAudioSink && Pipewire.defaultAudioSink.audio && Pipewire.defaultAudioSink.audio.muted
    function batteryIcon() {
        const d = UPower.displayDevice;
        if (!d || !d.isPresent) return "power";
        if (d.state === UPowerDeviceState.Charging || d.state === UPowerDeviceState.PendingCharge)
            return "battery_charging_full";
    ...
    Repeater {
        model: [
            { ic: "wifi",          fill: 0, menu: 0 },
            { ic: root.muted ? "volume_off" : "volume_up", fill: root.muted ? 0 : 1, menu: 1 },
            { ic: "bluetooth",     fill: 0, menu: 2 },
            { ic: root.batteryIcon(), fill: 1, menu: 3 },
            { ic: "notifications", fill: 0, menu: 4 }
        ]
        delegate: Rectangle {
            id: statusItem
            required property var modelData
            ...
            MSym {
                ...
                size: 21
                fill: modelData.fill
                color: statusItem.selected ? root.theme.accent : root.theme.textDim
            }
            MouseArea {
                id: statusMa
                anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
...
Rectangle {
    ...
    Column {
        ...
        Text {
            ...
            font.family: root.theme.mono
            color: root.theme.faint; font.pixelSize: root.theme.fsSmall
            font.letterSpacing: root.theme.labelSpacing; font.capitalization: Font.AllUppercase
            bottomPadding: 5
        }
        Repeater {                        // launcher actions (New Window, …)
            ...
            delegate: Rectangle {
                required property var modelData
                width: ctxCol.width; height: 30; radius: root.theme.radiusBtn
                color: ma.containsMouse ? root.theme.rowHi : "transparent"
                Text {
                    anchors.verticalCenter: parent.verticalCenter; x: 10
                    font.family: root.theme.family
                    text: modelData.name; color: root.theme.text; font.pixelSize: root.theme.fsNormal
        ...
        Repeater {                        // this app's windows (click to focus)
            ...
            delegate: Rectangle {
                required property var modelData
                width: ctxCol.width; height: 30; radius: root.theme.radiusBtn
                color: wma.containsMouse ? root.theme.rowHi : "transparent"
                Text {
                    anchors.verticalCenter: parent.verticalCenter; x: 10
...
Variants {
    ...
    PanelWindow {
        ...
        Item {
            ...
            anchors.horizontalCenter: parent.horizontalCenter
            y: 6
            width: win.showBurst ? (burstLoader.item ? burstLoader.item.implicitWidth : 96) : win.launcher ? win.launcherWidth : (win.notifMorph ? (morphStack.item ? morphStack.item.implicitWidth : 420) : (win.open ? 520 : (win.dash ? win.hoverWidth : (56 + root.privacyCount * 20 + (notifs.grouped.length > 0 ? 14 : 0)))))
            height: win.showBurst ? 28 : (win.open || win.launcher) ? win.openHeight : (win.notifMorph ? morphStack.implicitHeight : (win.dash ? win.hoverHeight : 28))
            ...
            Rectangle {
                ...
                color: theme.bgHover
                opacity: (win.dash && !win.open && !win.launcher && !win.notifMorph && target) ? 1 : 0
                visible: opacity > 0
                x: pos.x - pad
                y: pos.y - pad
            ...
            Item {
                ...
                height: 36
                opacity: (win.dash && !win.open && !win.launcher && !win.notifMorph) ? 1 : 0
                visible: opacity > 0
