...
the Claude polish model (no text fields, so no keyboard-grab complications), and
a hint pointing at the editable prompt files under
=~/.config/quickshell/transcribe/=.
*** Code
    ...
    Column {
        ...
        Flickable {
            ...
            Column {
                ...
                VolLabel { text: "Claude Polish"; visible: root.tab === 2 }
                CycleRow {
                    visible: root.tab === 2
                    label: "Model"
                ...
                Text {
                    ...
                    width: col.width
                    wrapMode: Text.Wrap
                    text: "Prompts (editable): ~/.config/quickshell/transcribe/whisper-prompt.md · polish-prompt.md — defaults are written on first run."
                    color: root.theme.faint
                    font.family: root.theme.family
                    font.pixelSize: root.theme.fsSmall
                }
...
Call shape: =voicebridge.py WAV MODE MODEL LANGUAGE DEVICE VAD CLAUDE_MODEL=
(=MODE= raw|polish, =LANGUAGE= auto|ISO code, =DEVICE= auto|cuda|cpu, =VAD=
vad|novad), or =voicebridge.py init= to just write the default prompt files.
...
- *API key*: =ANTHROPIC_API_KEY= if set, else keyring =anthropic/pill-transcribe=
  (KWallet via the venv's =keyring=; stored by the setup script).
...
def api_key():
    ...
    try:
        import keyring
        return keyring.get_password("anthropic", "pill-transcribe") or ""
    except Exception:
        return ""
...
def polish(text, claude_model):
    import anthropic
    key = api_key()
    if not key:
        raise RuntimeError("no API key — store one via setup-transcribe.sh "
                           "(keyring anthropic/pill-transcribe) or set ANTHROPIC_API_KEY")
    client = anthropic.Anthropic(api_key=key)
    resp = client.messages.create(
        model=claude_model,
        max_tokens=4000,
        system=read_prompt(POLISH_PROMPT),
...
def main():
    if len(sys.argv) >= 2 and sys.argv[1] == "init":
        ...
        return
    wav, mode, model, language, device, vadflag, claude_model = sys.argv[1:8]
    ensure_prompts()
    ok, err = True, ""
    try:
...
5. Prompts for the Anthropic API key and stores it in the keyring
   (=anthropic/pill-transcribe=, KWallet-backed). Blank skips — raw
   transcription works without a key; only polish needs it.
6. Warns if the RNNoise LADSPA plugin (=noise-suppression-for-voice=) is
...
echo "==> Anthropic API key for the polish pass (stored in keyring; blank = skip,"
echo "    raw transcription works without one)"
read -rsp "    API key: " KEY; echo
if [ -n "${KEY:-}" ]; then
    "$VENV/bin/python" - "$KEY" <<'PY'
import sys, keyring
keyring.set_password("anthropic", "pill-transcribe", sys.argv[1])
print("    key stored (keyring anthropic/pill-transcribe)")
PY
fi

echo "==> done. Toggle with Meta+X (raw) / Meta+Shift+X (polish)."
#+end_src
...
IpcHandler {
    ...
    function voiceToggle(): void       { root.voiceRequest("raw"); }
    function voicePolishToggle(): void { root.voiceRequest("polish"); }
}
...
property string voiceMode: "raw"       // "raw" | "polish" — picked at STOP time
...
// both shortcuts land here: start when idle, stop-with-route when recording,
// no-op while transcribing (double-toggle guard).
function voiceRequest(mode) {
    if (voiceState === "recording") voiceStop(mode);
    ...
    onExited: {
        ...
        if (root.voiceState === "transcribing") {
            ...
            voiceTranscribeProc.command = [
                ...
                root.voiceWavPath, root.voiceMode,
...
FileView {
    ...
    JsonAdapter {
        ...
        property string claudeModel: "claude-opus-4-8"  // polish-pass model
    }
}
#+end_src
...
Variants {
    ...
    PanelWindow {
        ...
        Connections {
            ...
            // over the recorder (voiceMorph is gated on !dash); open panels and
            // the launcher are left alone — the recorder just waits them out.
        ...
        Item {
            ...
            Loader {
                ...
                sourceComponent: VoiceRecorderPill {
                    ...
                    onStopRequested: root.voiceStop("raw")
                }
...
Component { id: cVol;  VolumeMenu    { theme: theme; settings: settings } }
