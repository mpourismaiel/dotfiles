...
def run_action(ids_json, verb):
    ...
    js = (
        "var TARGETS = %s;\nvar VERB = %s;\n%s"
        % (json.dumps(ids), json.dumps(verb), ACTION_JS)
    )
...
/ battery / notifications icons. The glyphs track live state — the volume icon
...
// is highlighted. Menu ids: 0 net, 1 vol, 2 bt, 3 batt, 4 notif. The hover highlight
...
Variants {
    ...
    PanelWindow {
        ...
        Item {
            ...
            height: win.showBurst ? 28 : (win.open || win.launcher) ? win.openHeight : win.ctxMode ? Math.min(win.openHeight, ((ctxLoader.item || trayLoader.item) ? (ctxLoader.item || trayLoader.item).implicitHeight + theme.pad * 2 : 300)) : (win.notifMorph ? morphStack.implicitHeight : (win.dash ? win.hoverHeight : 28))
