...
=loginctl= and =org.kde.Shutdown= over =gdbus=. Grid mode + favourite ids persist
...
honoured, and =expireTimeout === 0= ("persistent") is treated as the 5s cap too.
Only =Critical= popups are permanent (errors shouldn't vanish on their own).
...
QtObject {
    ...
    readonly property int defaultTimeoutMs: 5000   // hard ceiling: no timed popup outlives this
    function timeoutFor(n) {
        if (!n) return 0;
        // Critical alone is permanent — errors shouldn't auto-dismiss.
        if (n.urgency === NotificationUrgency.Critical) return 0;
        // everything else is timed and capped at 5s: honour a *shorter* app-requested
        // timeout, but ignore longer ones and treat 0 ("persistent") / -1 ("default")
        // as the cap too, so nothing non-critical lingers past defaultTimeoutMs.
...
summary + close-X with a vertical countdown bar, body (=StyledText=, links open
    ...
    Column {
        ...
        Item {
            ...
            Row {
                ...
                Rectangle {                          // close / dismiss button
                    anchors.verticalCenter: parent.verticalCenter
...
Once more than one popup is active a floating *"Dismiss all"* pill appears top-right
and calls =notifs.dismissAll()= (closes every popup, keeps them in history). Used
...
1. *collapsed* (96×28): =hh:mm=.
2. *hovered* (=dash=, 384×104): two balanced rows — Row 1 desktop dots (left) |
...
4. *launcher* (520×470): the launcher takes the menu pane's place (see
        ...
        PanelWindow {
            ...
            Item {
                ...
                width: win.showBurst ? (burstLoader.item ? burstLoader.item.implicitWidth : 96)
                     : win.launcher ? 520 : (win.notifMorph ? 420 : (win.dash ? 420 : (56 + root.privacyCount * 20)))
                ...
                Row {
                    ...
                    Text {
                        ...
                        // full opacity while a camera/mic is live (the pill is solid
                        // then); otherwise slightly translucent so the floating pill
                        // reveals a hint of what's behind it (readable on any backdrop)
                        color: win.privacyRest ? theme.text
                             : Qt.rgba(theme.text.r, theme.text.g, theme.text.b, 0.78)
                        font.family: theme.mono
                        font.pixelSize: theme.fsNormal
                    }
                    // camera / mic indicators — pop in (and shrink out) like the
                    // action bursts: scale overshoots on appear (OutBack), the slot
                    // width animates 0<->14 so the clock slides over smoothly, and the
                    // pill's own width Behavior widens to match. Static model so the
                    // delegates persist (a reactive model would rebuild and skip the
                    // animation); each reads its live state by index.
                    Repeater {
                        model: [
                        ...
                        delegate: Item {
                            ...
                            required property int index
                            required property var modelData
                            readonly property bool on: index === 0 ? root.cameraInUse : root.micActive
                            anchors.verticalCenter: parent.verticalCenter
                            ...
                            IconImage {
                                ...
                                Behavior on opacity { NumberAnimation { duration: theme.animFast } }
                                Behavior on scale  { NumberAnimation { duration: theme.anim; easing.type: Easing.OutBack } }
                            }
                        }
                    }
                }
                        ...
                        Repeater {
                            ...
                            delegate: Item {
                                ...
                                Rectangle {                  // hover highlight
                                    ...
                                    anchors.margins: -4
                                    radius: theme.radiusSmall
                                    color: appMa.containsMouse ? theme.bgHover : "transparent"
                                    Behavior on color { ColorAnimation { duration: theme.animFast } }
                                }
                                IconImage {
                                    anchors.top: parent.top
                ...
                Item {
                    ...
                    Row {                                    // launcher button + date/time (left)
                        ...
                        anchors.verticalCenter: parent.verticalCenter
                        spacing: 10
                        Rectangle {                          // open the app launcher
                            anchors.verticalCenter: parent.verticalCenter
                            width: 32; height: 26; radius: theme.radiusSmall
                            ...
                            MouseArea { id: launchMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                    ...
                    Row {                                    // status icons + system tray (right)
                        ...
                        Repeater {                           // status icons -> open panel to that menu (active one highlighted)
                            ...
                            delegate: Rectangle {
                                required property var modelData
                                width: 32; height: 26; radius: theme.radiusSmall
                                color: (win.open && win.menu === modelData.menu) ? theme.accentDim
                                       : statusMa.containsMouse ? theme.bgHover : "transparent"
                                Behavior on color { ColorAnimation { duration: theme.animFast } }
                                IconImage {
                                    anchors.centerIn: parent
                                    implicitSize: 18
                                ...
                                MouseArea { id: statusMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
