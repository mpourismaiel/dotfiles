...
(=financeRequested()=). All three finance signals hide while privacy is on.
*** Code
...
=loadForecastRange= (calendar dots: filled for real entries, hollow for
forecasts), =loadAccounts= / =loadBalances= / =loadTimeline= / =loadRegister= /
...
QtObject {
    ...
    property var wishlist: ({ liquid: [], items: [] })
    ...
    function bridge(args) {
        return ["python", Quickshell.shellPath("hledgerbridge.py")].concat(args);
    }
    function loadDay(key) {
        root.dayKey = key;
    ...
    function loadForecastRange(a, b) {
        forecastRangeProc.command = root.bridge(["frange", a, b]);
    ...
    function loadWishlist() {
        wishlistProc.command = root.bridge(["wishlist"]);
    ...
    Component.onCompleted: checkToday()
    ...
    property Process wishlistProc: Process {
        stdout: StdioCollector {
            onStreamFinished: {
                try { root.wishlist = JSON.parse(this.text) || { liquid: [], items: [] }; }
                catch (e) { root.wishlist = { liquid: [], items: [] }; }
            }
        }
    }
...
Item {
    ...
    // ---- mode machine ----
    property string mode: "cal"               // cal | add | forecast | register | balances | wishlist
    ...
    Item {
        ...
        Text {
            id: liquidHead
            text: {
                var l = (root.fin && root.fin.wishlist.liquid) ? root.fin.wishlist.liquid : [];
                if (!l.length || !root.fin) return "Liquid: —";
                var parts = [];
                for (var i = 0; i < l.length; i++)
                    parts.push(root.fin.fmtAmount(l[i].value, l[i].currency));
...
#   wishlist           print JSON {liquid, items: [{description, amount, currency, affordable}]}
...
FINANCE_DIR = os.path.expanduser(os.environ.get("PILL_FINANCE_DIR",
                                                "~/Documents/finance"))
...
def cmd_wishlist():
    liquid = balance_data(["assets"])["totals"]
    have = {a["currency"]: a["value"] for a in liquid}
    items = []
    for t in print_json([], WISHLIST):
        ...
        if not wish:
            continue
        cur = wish[0]["currency"]
        price = sum(p["value"] for p in wish if p["currency"] == cur)
        items.append({"description": row["description"], "amount": price,
                      "currency": cur,
                      "affordable": (have[cur] >= price) if cur in have else None})
    return {"liquid": liquid, "items": items}
...
Row {
    ...
    // shown while screen-recording — when the app-icon strip is hidden to avoid
    // revealing notification sources on a shared screen. Slides in/out like the
    // privacy icons (animated slot width).
    Item {
        readonly property bool showDot: root.hasUnread && root.recordingOn
        anchors.verticalCenter: parent.verticalCenter
        width: showDot ? 6 : 0
        height: 6
    ...
    // (recording / camera) indicators. Hidden while screen-recording so the shared
    // screen doesn't reveal notification sources — the fallback unread dot covers
    // that case instead. Capped at `iconsMax` with a "+N" overflow chip.
    Row {
        id: appStrip
        anchors.verticalCenter: parent.verticalCenter
        spacing: 4
        ...
        readonly property var shown: root.recordingOn
            ? [] : root.notifGroups.slice(0, root.iconsMax)
        readonly property int overflow: root.recordingOn
...
// are waiting. Capped at `notifAppIconsMax` with a "+N" overflow chip. While a
// screencast is live the app icons are hidden (don't reveal notification sources
// on a shared screen) and a single generic unread dot stands in. `notifRestWidth`
// is the extra pill width that footprint needs (used in the resting width below).
readonly property int notifAppIconsMax: 4
readonly property int notifRestWidth: {
    if (!notifs || notifs.grouped.length === 0) return 0;
    if (root.screenRecording) return 14;                 // generic unread dot only
...
Component { id: cCal;  CalendarMenu  { theme: theme; org: orgAgenda; fin: finance } }
