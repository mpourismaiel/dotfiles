...
| Action bursts (desktop switch / layout change)     | ✅ built       | 1s transient takeover of the resting pill (=root.burstKind=): desktop dot slides old→new; layout shows =US → FA=. Layout via KWin DBus =org.kde.keyboard /Layouts=; coexists with notifications (deck floats below) |
...
3. *open* (470 tall): control panel. Both dashboard rows *stay visible* on top —
   ...
   menu's own toggles. Clicking the (hovered) pill body opens the *app launcher*;
   a Row-2 status icon instead opens straight to its own menu (and switches it
   while open). =win.menu= still defaults to =4= (notification history) for when a
   menu is opened without first picking an icon.
4. *launcher* (=launcher=, 520×470): the app launcher takes the menu pane's place,
   *also below the two rows* (same divider + offset), so it matches the menus (see
   ...
   (=view-grid-symbolic=), by clicking the hovered pill body, or by the *Meta+H*
   global shortcut.

*Keyboard focus.* The layer surface only grabs the keyboard while a panel is open:
=launcher= → =WlrKeyboardFocus.Exclusive= (so it focuses even when opened by the
shortcut — there's no click to trigger on-demand focus — and arrow-key nav works);
a =menu= (=open=) → =OnDemand= (a password field focuses when clicked); otherwise
=None=. KWin does *not* restore focus to the previously-active window when a layer
surface releases the keyboard, so the pill remembers the last real active window
(=root.lastActiveWindow=, updated from =winbridge='s active-window reports, ignoring
the transient empties an open grab causes) and =restoreFocus()= re-activates it via
the KRunner =Run= whenever the pill is *dismissed without launching* (backdrop
click, launcher/menu back-chevron, Escape, shortcut toggle-close). Launching an app
skips the restore — the launched window takes focus itself.
...
** Global shortcut (Meta+H)
Quickshell's =GlobalShortcut= is Hyprland-only, so the launcher shortcut rides the
KWin script + DBus we already have (see =winbridge.py=): the script
=registerShortcut("PillLauncher", …, "Meta+H", …)= calls our =org.kde.pill=
=Launcher(screen)= method (passing the active window's =output.name=); =winbridge=
prints a =\{event:"launcher", screen\}= line; =init.qml= turns it into the
=launcherShortcut(screen)= signal, and each monitor's pill toggles its launcher if
the screen matches (single-monitor setups ignore the hint). The shortcut is
re-bindable in *System Settings ▸ Shortcuts ▸ KWin* once registered.
...
prints one JSON line =\{active, fullscreen, windows\}= per update on stdout (plus the
occasional =\{event:"launcher", screen\}= line for the Meta+H shortcut), which a
=Process= + =SplitParser= parses into =root.windows= / =root.activeWindow= /
...
- *launcher shortcut*: the same script =registerShortcut=s *Meta+H*, whose callback
  calls =org.kde.pill Launcher(activeWindow.output.name)= → the =\{event:"launcher"\}=
  line above (see [[*Global shortcut (Meta+H)][§ Global shortcut]]).
...
=wl-copy='s on click/Enter). The visible list/grid is *arrow-key navigable* from
the search field (=currentIndex= highlights a row/tile; =moveVert=/=moveHoriz= step
it, =activateCurrent= launches on Enter): up/down always move the selection; in the
grid left/right move a column too, while in a list left/right only navigate when the
query is empty — otherwise they move the text cursor. In *grid mode* the favourites
grid is
*drag-to-reorder*: tiles wrap a =DelegateModel= whose =items= group is moved live
as a tile is dragged over its neighbours (stable delegates + animated displace),
and the new order is committed to =settings.favorites= on release (the all-apps
grid, shown when there are no favourites, is not reorderable). A small *power row*
sits bottom-right — lock/logout/reboot/shutdown (16px), wired to =loginctl
lock-session= and =org.kde.Shutdown='s =logout=/=logoutAndReboot=/=logoutAndShutdown=
over =gdbus=. =init.qml= focuses + clears the field on each open and *drops the
field's focus on close* (so the pill releases the keyboard) via a =Connections= on
=win.launcher=.
...
=NotificationHistory= (menu 4). A momentary *action burst* (next section) also
suppresses the morph for its one second and floats the deck below, so the
notifications visibly *slide down* to make room for the burst pill.

** Action bursts (desktop switch, layout change)
Some changes deserve a glance even when the pill is at rest, so a *transient
burst* briefly takes over the collapsed pill for *1s* and then reverts to the
clock. It is driven from =root= (so every monitor's pill shows it) via
=burstKind= / =burstPhase=: =startBurst()= runs a single =Timer= through three
phases — *settle → animate 400ms → settle 300ms* — clearing =burstKind= when done.
The opening settle is *300ms* for the language burst but only *100ms* for the
desktop one (so the dots snap in, then the active dot slides). A burst only takes over the *resting* pill (=showBurst=): it yields to
hover / open / launcher / a right-click group (those already show richer UI), and
it overrides =notifMorph= so an active notification deck *floats below* the burst
pill instead of being replaced (the same below-pill =Loader= used for an opened
pill) — that is how notifications slide down to reveal the change. Even under a
focused fullscreen window the burst still *draws* (its input region stays empty,
so it never intercepts clicks). Two events fire bursts today, each guarding
against its *initial* value on startup (QML change handlers carry no old value, so
=root= keeps the previous desktop id / layout index to animate *from*):
- *Virtual-desktop switch* (=onCurrentDesktopChanged=) — renders the same dot row
  as Row 2 in its unhovered styling, with a filled =textDim= dot that slides from
  the previous desktop's slot to the new one across the 400ms animate phase
  (=burstDotIdx= flips to the new index once animating; a =Behavior on x= does the
  move). The *hovered* Row-2 dots use the same trick: hollow dots plus one filled
  dot that slides to =deskActiveIdx= (=Behavior on x=, 400ms =OutCubic=), so
  switching desktops while the pill is open animates instead of jumping.
- *Keyboard-layout change* (=onKbIndexChanged=) — shows =from → to= (e.g. =US →
  FA=) with the accent arrow *flashing and pulsing* during the animate phase.
  Layouts come from KWin's =org.kde.keyboard /Layouts= (=getLayoutsList= →
  =a(sss)= = code / custom-label / long-name; =getLayout= → current index;
  =layoutChanged= / =layoutListChanged= keep them live). The label is the user's
  *custom* layout label if set in System Settings, else the uppercased code (=us=
  → =US=, =ir= → =IR=).
...
| =init.qml=    | windows, stages, virtual desktops, taskbar, tray, action bursts, menu host + launcher (entry) |
        ...
        Item {
            ...
            ListView {
                ...
                delegate: Rectangle {
                    ...
                    color: appRow.index === root.currentIndex ? root.theme.accentDim
                         : rowMa.containsMouse ? root.theme.bgHover : "transparent"
            ...
            GridView {
                ...
                moveDisplaced: Transition { NumberAnimation { properties: "x,y"; duration: root.theme.animFast } }
                displaced: Transition { NumberAnimation { properties: "x,y"; duration: root.theme.animFast } }
                ...
                model: DelegateModel {
                    ...
                    delegate: MouseArea {
                        ...
                        onPressed: {
                            if (grid.canReorder) {
                                grid.dragOrder = root.favApps.map(a => a.id);
                        ...
                        DropArea {                       // reorder as a tile is dragged over
                            anchors.fill: parent
                        ...
                        Rectangle {
                            ...
                            color: tile.index === root.currentIndex ? root.theme.accentDim
                                 : tile.containsMouse ? root.theme.bgHover : "transparent"
                            ...
                            states: State {
                                ...
                                ParentChange { target: tileBg; parent: grid }
                                AnchorChanges {
                                    target: tileBg
...
KWIN_JS = (
    ...
    "pushList();\n"
    # global shortcut (Meta+H) to toggle the launcher; passes the focused window's
    ...
    'registerShortcut("PillLauncher","Pill: open launcher","Meta+H",function(){\n'
...
ShellRoot {
    ...
    // requested via the Meta+H global shortcut (KWin script -> winbridge -> here)
    ...
    Process {
        ...
        stdout: StdioCollector {
            // seed prevKbIndex here too: kbIndex defaults to 0, so if the fetched
            // value is also 0 the change handler never fires to consume the initial
            // value — without this the first *real* switch would be swallowed.
            onStreamFinished: {
                const m = this.text.match(/\d+/);
                if (m) { root.kbIndex = parseInt(m[0]); if (root.prevKbIndex === -1) root.prevKbIndex = root.kbIndex; }
            }
        }
    }
    Process {
        ...
        stdout: SplitParser {
            onRead: line => {
                if (line.indexOf("layoutChanged") !== -1) {
                    const m = line.match(/\(\s*(?:uint32\s+)?(\d+)\s*\)/);
    ...
    // changed, then reverts: phase 1 settle (300ms) -> phase 2 animate (400ms)
    // -> phase 3 settle (300ms) -> done. Driven from root so every monitor's
    // pill shows it; the pill suppresses notifMorph while bursting and floats
    // the notification deck below instead (so notifications slide down).
    property string burstKind: ""      // "" | "desktops" | "language"
    ...
    function startBurst(kind) { burstKind = kind; burstPhase = 1; burstTimer.restart(); }
    Timer {
        id: burstTimer
        // settle -> animate (400) -> settle; the desktop burst opens with a shorter
        // 100ms settle (language keeps 300ms), so it snaps in before the dot slides.
        interval: root.burstPhase === 2 ? 400
                : root.burstPhase === 1 ? (root.burstKind === "desktops" ? 100 : 300)
                : 300
        repeat: false
        onTriggered: {
            if (root.burstPhase < 3) { root.burstPhase++; burstTimer.restart(); }
            else { root.burstPhase = 0; root.burstKind = ""; }
        }
    }
    ...
    Variants {
        ...
        PanelWindow {
            ...
            // *Exclusive* so it grabs the keyboard even when opened by the Meta+H
            ...
            WlrLayershell.keyboardFocus: win.launcher ? WlrKeyboardFocus.Exclusive
                                       : win.open ? WlrKeyboardFocus.OnDemand
                                       : WlrKeyboardFocus.None

            // toggle the launcher (used by the Meta+H global shortcut). Restores
            // focus to the previously-active window when the shortcut closes it.
            function toggleLauncher() {
                if (win.launcher) { win.launcher = false; root.restoreFocus(); }
                else { win.open = false; win.ctxGroup = null; win.menu = 4; win.launcher = true; }
            }
            Connections {
                target: root
                function onLauncherShortcut(screen) {
                    ...
                    if (screen !== "" && Quickshell.screens.length > 1
                            && win.screen && screen !== win.screen.name) return;
                    win.toggleLauncher();
                }
            }
            ...
            property bool showBurst: root.burstKind !== "" && !dash
            ...
            MouseArea {
                ...
                onClicked: { win.open = false; win.launcher = false; win.ctxGroup = null; root.restoreFocus(); }
            }
            ...
            Item {
                ...
                anchors.horizontalCenter: parent.horizontalCenter
                y: 6
                width: win.showBurst ? ((burstLoader.item ? burstLoader.item.implicitWidth : 80) + 28)
                     : win.launcher ? 520 : (win.notifMorph ? 420 : (win.dash ? 420 : 56))
                height: win.showBurst ? 28
                      : (win.open || win.launcher) ? 470
                ...
                opacity: win.showBurst ? 1 : (win.fsHide ? 0 : ((win.dash || win.notifMorph) ? 1 : theme.idleOpacity))
                ...
                MouseArea { anchors.fill: parent; enabled: win.dash && !win.open && !win.launcher && !win.notifMorph; onClicked: { win.launcher = true; } }
                ...
                // Replaces the collapsed clock for 1s; content picked by burstKind.
                Loader {
                    id: burstLoader
                    anchors.centerIn: parent
                    ...
                    Row {                                    // launcher button + date/time (left)
                        ...
                        Item {
                            ...
                            Rectangle {                          // active indicator, slides on switch
                                ...
                                y: 0
                                color: theme.textDim
                                visible: root.deskActiveIdx >= 0
                                x: Math.max(0, root.deskActiveIdx) * 12
                ...
                Item {
                    ...
                    Item {                            // menu pane (full width)
                        ...
                        Connections {                 // chevron back -> collapse panel
                            ...
                            function onCloseRequested() { win.open = false; root.restoreFocus(); }
                        }
                    }
                }
                ...
                Item {
                    ...
                    Launcher {
                        ...
                        onCloseRequested: { win.launcher = false; root.restoreFocus(); }
                    }
                    // focus the search field on open; release it on close so the
                    // pill drops the keyboard grab and the active window gets it back
                    Connections {
                        target: win
                        function onLauncherChanged() { if (win.launcher) launcherItem.open(); else launcherItem.close(); }
                    }
                }
            }
    ...
    // ---- action-burst content (reads root.burst* / theme by id) ----
    // language: prevLabel -> (flashing arrow) -> newLabel.
    // desktops: the same dot row as Row 2, with a filled dot that slides from the
    // previous desktop's slot to the new one during the 0.3s animate phase.
    Component {
        id: cBurst
        Item {
            implicitWidth: root.burstKind === "language" ? langRow.implicitWidth : dotRow.width
            implicitHeight: 28

            Row {                                    // language: EN -> FA
                id: langRow
                anchors.centerIn: parent
                visible: root.burstKind === "language"
                spacing: 7
                Text {
                    anchors.verticalCenter: parent.verticalCenter
                    text: root.burstFromLabel; color: theme.textDim
                    font.family: theme.family; font.pixelSize: theme.fsNormal; font.bold: true
                }
                Text {                               // arrow flashes during the animate phase
                    anchors.verticalCenter: parent.verticalCenter
                    text: "→"; color: theme.accent
                    font.family: theme.family; font.pixelSize: theme.fsNormal; font.bold: true
                    opacity: root.burstPhase === 2 ? 1 : 0.3
                    Behavior on opacity { NumberAnimation { duration: 110 } }
                    SequentialAnimation on scale {
                        running: root.burstPhase === 2; loops: 2
                        NumberAnimation { from: 1.0; to: 1.35; duration: 100 }
                        NumberAnimation { from: 1.35; to: 1.0; duration: 100 }
                    }
                }
                Text {
                    anchors.verticalCenter: parent.verticalCenter
                    text: root.burstToLabel; color: theme.text
                    font.family: theme.family; font.pixelSize: theme.fsNormal; font.bold: true
                }
            }

            Item {                                   // desktops: dots with a sliding active dot
                anchors.centerIn: parent
                visible: root.burstKind === "desktops"
                width: dotRow.width; height: 8
                Row {
                    id: dotRow
                    spacing: 4                        // matches the Row-2 desktop dots (pitch 12)
                    Repeater {
                        model: root.desktops
                        delegate: Rectangle {
                            width: 8; height: 8; radius: 4
                ...
                Rectangle {                          // the moving active dot
                    width: 8; height: 8; radius: 4
                    y: 0
                    color: theme.textDim
                    x: root.burstDotIdx * 12
                    Behavior on x { NumberAnimation { duration: 400; easing.type: Easing.OutCubic } }
                }
            }
        }
