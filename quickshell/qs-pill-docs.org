...
=MenuHeader= title and a *Today* button in the header's trailing slot next to a
*Gregorian ⇄ Shamsi* toggle. Gregorian is primary (the big figure); the Jalaali
            ...
            Repeater {
                ...
                delegate: Item {
                    ...
                    MouseArea {
                        ...
                        onClicked: root.select(cell.modelData.key)
                    }
                }
            }
        }
                ...
                Repeater {
                    model: {
                        ...
                        items.sort(function (a, b) {
                            var at = (a.todo && a.todo.length) ? 0 : 1;
                            var bt = (b.todo && b.todo.length) ? 0 : 1;
                            return at - bt;
                        });
                        return items;
                    }
                    delegate: Row {
                        ...
                        // Clean the raw org-agenda line for the pill: the Emacs
                        // side already drops the category / "Deadline:" prefix,
                        // so here we only strip the leading TODO keyword (the dot
                        // already conveys it) and collapse any leftover padding.
                        readonly property string heading: {
                            var t = "" + (modelData.text || "");
                            var kw = modelData.todo || "";
                            if (kw.length)
                                t = t.replace(new RegExp("^\\s*" + kw + "\\s+"), "");
                        ...
                        Rectangle {
                            ...
                            anchors.top: parent.top
                            anchors.topMargin: 6
                            color: (agendaRow.modelData.todo && agendaRow.modelData.todo.length) ? root.theme.accent : root.theme.faint
                        }
                        Column {
                            width: agendaCol.width - 13
                            spacing: 2
                            Text {
...
QtObject {
    ...
    property var dayItems: []             // [{ text, todo, type }] for dayKey
...
=day YYYY-MM-DD= prints a JSON array of =\{text, todo, type\}= for that day's
scheduled/deadline/timestamp entries; =range A B= prints a JSON array of the =YYYY-MM-DD=
dates in =[A,B]= that have any entry (the calendar dots — one call per shown grid);
=open YYYY-MM-DD= raises an existing Emacs frame (or makes one) and opens the agenda.
...
#
#   day YYYY-MM-DD    print JSON [{text, todo, type}, …] for that day's entries
#   range A B         print JSON ["YYYY-MM-DD", …] of dates in [A,B] with entries
...
(progn
  ...
  (defun pill--day-entries (greg)              ; greg = (month day year)
    ;; Format entries for the pill, not for an Emacs agenda buffer: drop the
    ;; category + scheduling-info prefix ("inbox:  Deadline:  …") and the
    ;; right-aligned tag padding, so `text` is just the todo keyword + a clean
    ;; heading. The deadline/scheduled kind still rides along as the `type`
    ;; text property, which the pill re-surfaces in its own label style.
    ;; `org-prefix-format-compiled' is reset to nil so it recompiles from our
    ;; local `org-agenda-prefix-format' instead of a leftover agenda build.
            ...
            Item {
                ...
                Column {
                    ...
                    Item {
                        ...
                        readonly property int hoverPadY: 2
