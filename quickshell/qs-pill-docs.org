...
=Theme= is instantiated once in =pill.qml= and passed to each menu as a =theme=
property (no singletons, so nothing magic to set up).

** Window & input model (pill.qml)
...
** Virtual desktops (pill.qml, DBus)
...
| =pill.qml=        | windows, stages, virtual desktops, taskbar, tray, rail, menu host (entry) |

* Source

Edit these blocks; tangle to regenerate the files (see [[*Tangling][§ Tangling]]).
After tangling, validate with =qs -p quickshell/pill.qml=.

** Theme.qml
#+begin_src qml :tangle Theme.qml
...
** PillToggle.qml
#+begin_src qml :tangle PillToggle.qml
pragma ComponentBehavior: Bound
...
** PillSlider.qml
#+begin_src qml :tangle PillSlider.qml
pragma ComponentBehavior: Bound
...
** NetworkMenu.qml
#+begin_src qml :tangle NetworkMenu.qml
pragma ComponentBehavior: Bound
...
** VolumeMenu.qml
#+begin_src qml :tangle VolumeMenu.qml
pragma ComponentBehavior: Bound
...
** BluetoothMenu.qml
#+begin_src qml :tangle BluetoothMenu.qml
pragma ComponentBehavior: Bound
...
** BatteryMenu.qml
#+begin_src qml :tangle BatteryMenu.qml
pragma ComponentBehavior: Bound
...
#+begin_src python :tangle winbridge.py
#!/usr/bin/env python
...
Then validate:  =qs -p quickshell/pill.qml=  (Ctrl-C to stop). A clean load prints
