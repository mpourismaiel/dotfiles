            ...
            Repeater {
                ...
                delegate: Item {
                    ...
                    Rectangle {
                        ...
                        height: 4
                        radius: 2
                        color: cell.modelData.inMonth ? root.theme.accent : root.theme.faint
                    }
...
QtObject {
    ...
    property var rangeDays: []            // ["YYYY-MM-DD", …] within the loaded range that have entries
    ...
    function hasItems(key) { return root.rangeDays.indexOf(key) !== -1; }
    ...
    property Process rangeProc: Process {
        stdout: StdioCollector {
            onStreamFinished: {
                try { root.rangeDays = JSON.parse(this.text) || []; }
                catch (e) { root.rangeDays = []; }
            }
        }
    }
...
Rectangle {
    ...
    Column {
        ...
        Text {
            ...
            leftPadding: 8; bottomPadding: 5; topPadding: 3
        ...
        Repeater {                            // desktop actions (New Window, …)
            ...
            delegate: Rectangle {
                required property var modelData
                width: ctxCol.width; height: 30; radius: root.theme.radiusBtn
                color: actMa.containsMouse ? root.theme.rowHi : "transparent"
                Text {
        ...
        Rectangle {                           // favourite toggle
            width: ctxCol.width; height: 30; radius: root.theme.radiusBtn
            color: favMa.containsMouse ? root.theme.rowHi : "transparent"
            Row {
                anchors.verticalCenter: parent.verticalCenter; x: 10
                spacing: 8
                MSym {
                    ...
                    icon: "star"
                    size: 16
                    fill: (ctx.app && root.isFav(ctx.app)) ? 1 : 0
                    weight: 500
                    color: (ctx.app && root.isFav(ctx.app)) ? root.theme.accent : root.theme.faint
                }
                Text {
                    anchors.verticalCenter: parent.verticalCenter
...
Item {
    ...
    Column {
        ...
        component ClipRow: Rectangle {
            ...
            radius: root.theme.radiusRow
            color: rowMa.containsMouse ? root.theme.rowHi : root.theme.row
            clip: true
            Behavior on color { ColorAnimation { duration: root.theme.animFast } }
        ...
        Flickable {
            ...
            Column {
                ...
                Repeater {
                    model: root.shown
                    delegate: ClipRow { required property var modelData; width: listCol.width; entry: modelData }
                }
                Text {
...
(progn
    ...
    (let ((m (or (get-text-property 0 'org-hd-marker e)
      ...
      (if m
        ...
        (list (cons "text" (org-trim (substring-no-properties e)))
              ...
              (cons "dated" t)))))
  ;; One scan of all agenda headings, yielding two kinds of item the dated
  ;; agenda query doesn't cover (done handling is independent of the user's
  ;; org-agenda-skip-*-if-done settings, which otherwise hide finished tasks):
  ;;   • active undated TODOs (no SCHEDULED/DEADLINE) — the persistent to-do
  ;;     list, shown on every day;
  ;;   • any DONE heading — shown on its CLOSED day (log-mode style), so it
  ;;     appears once, on the day it was finished, and the pill sorts it last.
             ...
             (lambda ()
               (let* ((todo (org-get-todo-state))
                 ...
                 (cond
                  ...
                  ((and done closed (string-match-p (regexp-quote datestr) closed))
                   (append (pill--item)
                           (list (cons "type" "") (cons "dated" :json-false)))))))
             t 'agenda))))
  (defun pill-day (datestr)
    (json-encode
     (vconcat
      (append
       ;; active dated items from the agenda; done ones are dropped here and
       ;; re-added by pill--extra-items on their CLOSED day (avoids duplicates)
       (seq-remove (lambda (it) (eq (cdr (assoc "done" it)) t))
    ...
    (let* ((sa (calendar-absolute-from-gregorian (pill--parse startstr)))
      ...
      (while (<= a ea)
        (let ((g (calendar-gregorian-from-absolute a)))
          (when (pill--day-entries g)          ; dots track dated entries only
            (push (format "%04d-%02d-%02d" (nth 2 g) (nth 0 g) (nth 1 g)) out)))
        (setq a (1+ a)))
...
def main():
    ...
    if cmd == "day" and len(sys.argv) > 2:
        ...
        _emacs("(progn %s (pill-toggle %s %s))" % (HELPERS, _esc(sys.argv[2]), sys.argv[3]))
        sys.stdout.write("[]")
    elif cmd == "open":
...
// The hover highlight is NOT drawn here: each tile reports itself via itemEntered
// (on enter only) so the dashboard's single shared hover square jumps to it.
import QtQuick
import Quickshell
import Quickshell.Widgets
...
Row {
    ...
    // app icons are bigger than the menu/tray glyphs, so the app row is spaced
    // 1.5x the base icon spacing to keep the tiles from crowding.
    spacing: Math.round(root.theme.iconSpacing * 1.5)
    Repeater {
        model: root.groups
        delegate: Item {
            id: appItem
            ...
            width: root.theme.appCell; height: root.theme.appCell
            // ask the shared hover square for a little extra padding around app
            ...
            IconImage {
                ...
                source: appIcon.failed
                        ...
                        : Quickshell.iconPath(appItem.modelData.icon, "application-x-executable")
            ...
            Rectangle {                  // active / running indicator
                ...
                height: 2
                radius: 1
                width: appItem.active ? 20 : 7
                color: appItem.active ? root.theme.accent : root.theme.faint
            ...
            MouseArea {
                ...
                cursorShape: Qt.PointingHandCursor
                acceptedButtons: Qt.LeftButton | Qt.RightButton
                onContainsMouseChanged: if (appMa.containsMouse) root.itemEntered(appItem);
                onClicked: mouse => {
...
a camera icon while any process holds a =/dev/video*= node, a mic icon while the
default source is unmuted, and a pulsing red dot while the screen is being
shared/recorded (=recordingOn=; the dot emits a fading ring once every 5s). The
icons *pop in* / *shrink out* (scale overshoot via =OutBack=, an animated slot
width so the clock slides over). =solid= (a live camera/mic/recording) shows the
clock at full opacity; otherwise it is slightly translucent.
*** Code
#+begin_src qml :tangle ~/.config/quickshell/pill/CollapsedPill.qml :mkdirp yes
pragma ComponentBehavior: Bound
// CollapsedPill.qml — the collapsed pill: the hh:mm clock plus camera/mic/screen
// privacy indicators that pop in (OutBack scale + animated slot width so the clock
// slides over) while live. `solid` (a live camera/mic/recording) shows the clock at
...
Row {
    ...
    // Same pop-in / shrink-out slot behaviour as the camera/mic glyphs so the
...
=desktops=), *Minimize*, *Maximize*, *Keep Above/Below Others* — a separator, and a
*Close* row (these act on the whole group). The per-window actions and the whole-group
actions share one reusable =ActionBlock= sub-component.
...
uuids (a single window, or the whole group); =closeRequested()= backs out (host clears
=win.ctxGroup=). The host also clears =ctxGroup= after any window/launch action, so
acting closes the menu (Plasma behaviour). Content scrolls in a =Flickable=;
=implicitHeight= (header + rows, independent of the given height) drives the pill's
ctx-mode size.
*** Code
...
Item {
    ...
    component ActionBlock: Column {
        ...
        Row0 {
            visible: root.desktops.length > 0 && ab.deskExpanded
        ...
        Repeater {
            model: ab.deskExpanded ? root.desktops : []
            delegate: Row0 {
                required property var modelData
                indent: ab.indent + 26
        ...
        Row0 { indent: ab.indent; glyph: "minimize";                   label: "Minimize";          onAct: root.windowAction("minimize", ab.targets) }
        Row0 { indent: ab.indent; glyph: "crop_square";                label: "Maximize";          onAct: root.windowAction("maximize", ab.targets) }
        Row0 { indent: ab.indent; glyph: "keyboard_double_arrow_up";   label: "Keep Above Others"; onAct: root.windowAction("above", ab.targets) }
        Row0 { indent: ab.indent; glyph: "keyboard_double_arrow_down"; label: "Keep Below Others"; onAct: root.windowAction("below", ab.targets) }
        Sep {}
        Row0 { indent: ab.indent; glyph: "close"; label: "Close"; danger: true; onAct: root.windowAction("close", ab.targets) }
    }
...
explicitly. KDE shows its usual brightness OSD on the change.

*Taskbar.* =appGroups= groups =windows= (from =winbridge=) by *app class*, ordered
by *open time* (winbridge stamps each window with a stable open-order =seq=) — one
tile per app, a count badge for ≥2 windows, an underline that turns accent when any
of the app's windows is active. Each group's icon is resolved through
=DesktopEntries.heuristicLookup= (desktop-file name → class → KRunner icon name), so
tiles get a real icon instead of the generic fallback. Left-click =cycleGroup= focuses the app's next window;
right-click opens the [[*AppContextMenu.qml][=AppContextMenu=]] *inside the pill*
(like the control-panel menus, not a floating popup): the app name, its
=DesktopEntries.heuristicLookup(icon).actions=, then the KWin window actions (Move to
Desktop / Minimize / Maximize / Keep Above/Below / Close). Selecting a window action
...
holding a =/dev/video*= node open, polled every 2s with =fuser=), an *unmuted mic*
(the default Pipewire source), or an active *screen capture* (a =Stream/Output/Video=
PipeWire node published by xdg-desktop-portal / KWin while the screen is shared or
recorded). Any of these drops the idle translucency — the collapsed pill goes solid
(real surface + border, no gradient, full-opacity clock) — and shows an indicator
next to the clock: camera/mic as glyphs, screen recording as a *big red dot that
pulses (emits a fading ring) once every 5s*; they sit side by side when several are
live, widening the pill. The glyphs *pop in* and *shrink out* like the action bursts
(scale overshoots on appear via =OutBack=, the slot width animates so the clock
slides over). It also keeps the pill visible over a fullscreen window, where it would
otherwise hide.
...
IpcHandler {
    ...
    // step the *active monitor's* brightness up/down by ~5%. Bind these to KDE
    // custom-command shortcuts (see the brightness-shortcut note below); the active
    // monitor is the focused window's output (activeScreen, from winbridge).
    function brightnessUp(): void   { brightness.stepActive(root.activeScreen, 1); }
    function brightnessDown(): void { brightness.stepActive(root.activeScreen, -1); }
    // target a named monitor explicitly (skip the active-window lookup)
    function brightnessUpOn(screen: string): void   { brightness.stepActive(screen, 1); }
    function brightnessDownOn(screen: string): void { brightness.stepActive(screen, -1); }
}
...
=/dev/video*= node, polled with =fuser=), an unmuted default mic, and an active
*screen capture* (a =Stream/Output/Video= PipeWire node published by
xdg-desktop-portal / KWin while the screen is shared or recorded). Any of the three
makes the collapsed pill solid and shows an indicator (rendered by
[[*CollapsedPill.qml][=CollapsedPill=]]) — camera/mic as glyphs, screen recording
as a pulsing red dot.
#+name: init-privacy
#+begin_src qml

// ---- privacy indicators (resting pill) ----
// camera + mic + screen-capture in use show an icon next to the collapsed clock
// and make the resting pill opaque (drops the idle translucency) so it can't be
// missed.
...
readonly property bool privacyActive: cameraInUse || micActive || screenRecording
readonly property int  privacyCount: (cameraInUse ? 1 : 0) + (micActive ? 1 : 0) + (screenRecording ? 1 : 0)
Process {
    id: cameraProc
...
// group windows by app class (falling back to icon name, then uuid), ordered by
// open time. `windows` already arrives in open order (winbridge sorts by seq), so
// the first-seen group order is open order — keep it.
...
Variants {
    ...
    PanelWindow {
        ...
        property bool dash: open || launcher || hover.hovered || ctxMode
        ...
        // the clipboard menu takes Exclusive focus too (like the launcher) so its
        // search field grabs the keyboard even when opened by the IPC shortcut (no
        // click to trigger OnDemand focus); other menus stay OnDemand.
        WlrLayershell.keyboardFocus: (win.launcher || (win.open && win.menu === 5)) ? WlrKeyboardFocus.Exclusive : win.open ? WlrKeyboardFocus.OnDemand : WlrKeyboardFocus.None
        exclusionMode: ExclusionMode.Ignore
        color: "transparent"
        mask: (win.open || win.launcher || win.ctxMode || win.confirmActive) ? fullRegion : (win.fsHide ? emptyRegion : pillRegion)
        ...
        MouseArea {
            ...
            anchors.fill: parent
            enabled: win.open || win.launcher || win.ctxMode
        ...
        Item {
            ...
            height: win.showBurst ? 28 : (win.open || win.launcher) ? win.openPaneHeight : win.ctxMode ? Math.min(win.openHeight, ((ctxLoader.item || trayLoader.item) ? (ctxLoader.item || trayLoader.item).implicitHeight + theme.pad * 2 : 300)) : (win.notifMorph ? morphStack.implicitHeight : (win.dash ? win.hoverHeight : 28))
            ...
            Rectangle {
                anchors.fill: parent
                // collapsed pill is fully round; the dashboard / open panel uses
                // the Broadsheet panel radius.
                ...
                Column {
                    ...
                    Item {
                        ...
                        MouseArea {
                            ...
                            hoverEnabled: true
                            cursorShape: Qt.PointingHandCursor
                            onContainsMouseChanged: if (containsMouse) win.hoverEnter(dateTimeBlock)
            ...
            Item {
                ...
                Rectangle {
                    ...
                    MouseArea {
                        ...
                        hoverEnabled: true
                        cursorShape: Qt.PointingHandCursor
                        onContainsMouseChanged: if (launchMa.containsMouse) win.hoverEnter(launcherIcon)
                        onClicked: {
                            win.open = false;
                            win.launcher = true;
                        }
        ...
        // Clicking shutdown / reboot / logout opens this opaque page on top of
        // everything instead of firing the command. Four interchangeable designs
        // (1a Hush / 1b Blaze / 1c Ledger / 1d Split), ported from the Claude
        // mockups, are selected per-open by `theme.confirmation` ("1a".."1d" or
        // "random"); `win.confirmVariant` holds the resolved pick. Each design
        // renders the same data — the action word (shutdown/reboot/logout), the
        // random one-liner `confirmMsg`, and yes/no wired to win.confirm() /
        // win.cancelConfirm(). Escape (or "no") backs out; Enter confirms.
        // Fonts required: IBM Plex Mono, Newsreader, DM Serif Display,
        // Instrument Serif.
        Rectangle {
            id: powerConfirm

            // uppercased action word for the mono labels ("SHUTDOWN" etc.)
            readonly property string actionUpper: win.confirmAction.toUpperCase()

            anchors.fill: parent
            z: 1000
            color: "#09080a"
            opacity: win.confirmActive ? 1 : 0
            visible: opacity > 0
            focus: win.confirmActive
            Keys.onEscapePressed: win.cancelConfirm()
            ...
            Loader {
                ...
                sourceComponent: win.confirmVariant === "1b" ? cBlaze : win.confirmVariant === "1c" ? cLedger : win.confirmVariant === "1d" ? cSplit : cHush
            }
            ...
            Component {
                ...
                Item {
                    ...
                    Text {
                        ...
                        text: "SYSTEM · " + powerConfirm.actionUpper
                        color: "#3d130a"
                        font.family: "IBM Plex Mono"
                        font.pixelSize: 14
                    ...
                    Column {
                        ...
                        Item {
                            ...
                            Text {
                                ...
                                anchors.verticalCenter: parent.verticalCenter
                                text: "Confirm " + win.confirmAction
                                color: "#7a7076"
                                font.family: "IBM Plex Mono"
                                font.pixelSize: 13
                                font.letterSpacing: 4
