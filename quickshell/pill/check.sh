#!/bin/sh
# check.sh — validate the pill sources in this directory. Touches NOTHING live:
...
# PanelWindow delegate — see ../qs-pill-docs.org "Validating").
#
# Unset WAYLAND_DISPLAY for this run: when it is present (i.e. you run check.sh /
# deploy.sh from your live Wayland session) qs prints a benign "WAYLAND_DISPLAY is
# present but QT_QPA_PLATFORM is offscreen" WARNING, which the WARN filter below
# would otherwise flag as a failure. Offscreen wants no Wayland anyway.
LOG=$(mktemp)
