...
- `HabitState.qml` + `HabitMenu.qml` (menu 19) + `JungleView.qml` + `treegen.js` + `HabitDots.qml` + `HabitDialog.qml` + `FreezeDialog.qml` + `AddHabitDialog.qml` + `HabitField.qml` + `DateField.qml` + `PillCheckbox.qml` + `habiqbridge.py` — the habit tracker: the `habiq` CLI's plain-text journal rendered as a scrolling per-week jungle (deterministic species trees — oak/willow/pine/acacia/fern & co., curved tapered limbs + multi-tone canopies — one per habit per week; snow on frozen weeks, breeze sway, floor vegetation by weekly score, the whole garden one continuous iso lattice with a centered 5-cell separator row between weeks). Week legend syncs with scroll; right panel = per-habit cards for today; in-pill dialogs log/edit/delete entries (checkboxes for bool habits, a `DateField` masked-input-plus-calendar-popover for dates), manage freezes (one Freeze⇄Unfreeze button tracking the active state) and add/edit habit definitions (`habiq habit add`/`edit`). If the journal dir is a git repo, the header grows a **sync** control (like the finance strip): a status label reporting remote (`N↓`) / local (`N↑` / `N unsaved`) changes plus ONE button that pulls-then-pushes (commit → rebase onto remote → push; a real conflict is aborted untouched and reported, never auto-merged) via `habiqbridge.py`'s `git-status`/`git-sync`. Feature-gated via Settings → Habit Tracker (`settings.habitsEnabled` + journal dir); opened from plant buttons in the calendar/finance headers or the `h` jump.
