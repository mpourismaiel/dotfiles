...
- `HabitState.qml` + `HabitMenu.qml` (menu 19) + `JungleView.qml` + `treegen.js` + `HabitDots.qml` + `HabitDialog.qml` + `FreezeDialog.qml` + `AddHabitDialog.qml` + `HabitField.qml` + `habiqbridge.py` — the habit tracker: the `habiq` CLI's plain-text journal rendered as a scrolling per-week jungle (deterministic L-system trees, one per habit per week; snow on frozen weeks, breeze sway, floor vegetation by weekly score). Week legend syncs with scroll; right panel = per-habit cards for today; in-pill dialogs log/edit/delete entries, manage freezes and add habits. Feature-gated via Settings → Habit Tracker (`settings.habitsEnabled` + journal dir); opened from plant buttons in the calendar/finance headers or the `h` jump.
