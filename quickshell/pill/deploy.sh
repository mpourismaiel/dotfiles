#!/bin/sh
# deploy.sh — validate the pill sources (via ./check.sh), then copy them to the
# live Quickshell config at ~/.config/quickshell/pill/.
#
...
# (so the running shell reloads once, only when something actually changed).
#
...
set -eu
cd "$(dirname "$0")"
DEST="$HOME/.config/quickshell/pill"
...
if [ "$copied" -eq 0 ]; then
    ...
    echo "done: $copied file(s) copied — the running pill reloads on its own"
fi
