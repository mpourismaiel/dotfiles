...
* Credential setup
Prompts for an account name (a label of your choosing), your site URL (custom
domains welcome, used as-is) + API key, validates against the live API, and
stores them in the system keyring (never on disk in the repo). Run it once per
account: =~/.config/doom/teamwork/teamwork-setup.sh= (or, from Emacs,
=M-x teamwork-account-add=). Several accounts are supported — each is a separate
keyring item told apart by its name; =--print= lists them and =--remove NAME=
deletes one. When more than one account exists, the timesheet commands ask which
to use, and the pulled buffer records its choice in a =#+TEAMWORK_ACCOUNT= header
so submit targets the same account.
...
(=prefs_<account>.json= when an account is selected). Read/write subcommands take
=--account NAME=; snapshot cache and prefs are keyed per account so they never
clobber each other. `timesheet.py accounts` lists accounts as JSON;
`timesheet.py account-delete --account NAME` removes one from the keyring.
...
list w/ weekday + overlap + format warnings + action preview),
=teamwork-quit= (C-c C-k). Account management: =teamwork-account-add= (interactive
setup in a terminal), =teamwork-account-view= (list configured accounts),
=teamwork-account-delete= (pick one, confirm, remove from the keyring). When
multiple accounts are configured the pull commands prompt which to use.
