...
deployed together to `~/.config/doom/packages/github-pr/`.
...
```sh
python3 ~/.config/doom/packages/github-pr/test_github_pr.py
```
...
`config.org` loads it with `(mp/require-package "github-pr")`.
