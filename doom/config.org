...
**** Ren'Py
=renpy-mode= derives from =python-mode= and already autoloads itself for =.rpy=
files; this block just makes the association explicit so it survives any future
=auto-mode-alist= shuffling.
#+begin_src emacs-lisp
...
Pick a shell script from =__ignore__/scripts/= in the current project and run it
in a Ghostel terminal via =SPC p S=. Each project+script pair reuses its own
named terminal (=*project:NAME*=), so re-running a script sends the command to
the same buffer; scripts that have been launched this session are prefixed with
a ~✓~ in the picker.
#+begin_src emacs-lisp
...
(defun my/run-project-script ()
  "Pick a project script and run it in a Ghostel terminal."
  (interactive)
  (let* ((project (projectile-project-root))
    ...
    (let* ((choices
      ...
      ;; Open (or reuse, by buffer identity) a Ghostel named after the script
      ;; rooted at the project, then send the script path to its shell.
      (let ((default-directory project)
            (ghostel-buffer-name buf-name))
        (ghostel)
        (ghostel-send-string (concat (shell-quote-argument file) "\n"))
...
**** Org sort todos by state and priority
Reorder a TODO list so remaining work floats to the top: entries are grouped
by TODO state in the order they are defined in =org-todo-keywords= (TODO, NEXT,
WAIT, SOMEDAY, DONE, CANCELLED — headings with no keyword sort last), and within
each group by priority (=#A=, =#B=, =#C=, ..., then unprioritised). It builds a
single sort key like =00-A= per heading and hands it to =org-sort-entries=, so
one stable pass gives the state-then-priority ordering.

Scope is DWIM, in order of preference: an active region; the children of the
heading at point when it has any (put point on a list's parent heading); the
siblings of the heading at point (its parent's children — put point on any item
of a nested list); or every top-level heading when point is above the first one.
Bound to =SPC m s t=, next to Doom's generic =SPC m s S= (=org-sort=).
#+begin_src emacs-lisp
(defun mp/org-sort-todos ()
  "Sort a TODO list by TODO state, then by priority.

State order follows the definition in `org-todo-keywords'; headings with
no keyword sort last.  Within a state, higher priority first (#A, #B,
#C, ...), then unprioritised entries.

Scope, in order of preference: an active region; the children of the
heading at point when it has any; the siblings of the heading at point
\(its parent's children); or every top-level heading when point is above
the first one."
  (interactive)
  (let* ((getkey
          (lambda ()
            (let* ((comps (org-heading-components))
                   (todo  (nth 2 comps))
                   (prio  (nth 3 comps))
                   (state (or (seq-position org-todo-keywords-1 todo) 99)))
              (format "%02d-%s" state (if prio (char-to-string prio) "~")))))
         (do-sort (lambda () (org-sort-entries nil ?f getkey #'string<))))
    (cond
     ((org-region-active-p) (funcall do-sort))
     ((and (org-at-heading-p) (save-excursion (org-goto-first-child)))
      (funcall do-sort))
     ((save-excursion (and (org-at-heading-p) (org-up-heading-safe)))
      (save-excursion (org-up-heading-safe) (funcall do-sort)))
     (t
      ;; No parent to hang the sort on: select the whole buffer so
      ;; `org-sort-entries' takes its region branch and orders the
      ;; top-level headings.  Bind `transient-mark-mode' so the region
      ;; registers even when it is otherwise off.
...
(map! :after org
      ...
      (:prefix ("s" . "tree/subtree")
       :desc "Sort todos by state/priority" "t" #'mp/org-sort-todos))
#+end_src
