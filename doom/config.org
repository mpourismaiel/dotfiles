...
By default =agent-shell-notifications= ships a single "Switch to shell" action. The pill notification server already renders action buttons (and advertises the =actions= capability), so we enrich permission-request notifications with *Allow* / *Deny* buttons that answer the agent right from the notification.

The permission event itself doesn't carry the ACP option-ids needed to respond, and the package builds the action list internally. So we hook =agent-shell-permission-responder-function= — the one place that exposes both the option-ids and a ready-made =respond= closure — to stash them by request-id (returning nil so the normal in-buffer dialog still shows), then advise the plist builder to add the two extra actions and dispatch clicks to the stashed closure. A =:before= advice on the response sender forgets the entry once any path (buttons, in-buffer dialog, or interrupt) answers.
...
an existing auto-approve responder keeps working (nil means show the dialog)."
  ...
  (defun mp/agent-shell--enrich-plist (orig-fn type shell-buffer event)
    "Add Allow/Deny actions to permission-request notifications (:around advice).
The pill renders the first action as the accent button, so Allow leads."
    (let ((plist (funcall orig-fn type shell-buffer event)))
      (if-let* (((eq type 'permission-request))
                (request-id (map-nested-elt event '(:data :request-id)))
                (entry (gethash request-id mp/agent-shell--permission-registry))
                (respond (plist-get entry :respond))
                (options (plist-get entry :options))
                (allow-id (mp/agent-shell--option-id options "allow_once" "allow_always"))
                (deny-id (mp/agent-shell--option-id options "reject_once" "reject_always"))
                (switch (plist-get plist :on-action)))
          (progn
            (plist-put plist :actions
                       (list "allow" "Allow" "reject" "Deny" "default" "Switch to shell"))
