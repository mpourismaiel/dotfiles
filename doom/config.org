...
- ~[X] Actions~ :: rename the current file, delete it, or open a terminal in the
  directory currently being browsed. The path each action affects is shown on
  the right.
