...
**** Completion Styles
Use Orderless as the primary matcher for Vertico and Corfu, with the standard
fallbacks kept for file paths and completion-at-point providers.
...
| Global      | =M-d=      | =er/expand-region=              | Expand the active region semantically.                                      |
| Global      | =M-D=      | =er/contract-region=            | Contract the last region expansion.                                         |
...
| Leader      | =SPC c h=  | =eldoc-box-help-at-point=       | Show hover documentation on demand.                                         |
...
| Leader      | =SPC s r f=| =color-rg-search-input-in-current-file= | Search the current file with color-rg.                              |
| Leader      | =SPC s r p=| =color-rg-search-input-in-project= | Search the project with color-rg.                                       |
| Evil states | =z 1=…=z 9=| =mp/fold-to-level=              | Fold all regions to that nesting level (toggles open on repeat).            |
