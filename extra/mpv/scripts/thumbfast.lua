...
local function vf_string(filters, full)
  ...
  if full then
    vf =
      ...
      "scale=w=" ..
        effective_w ..
          ":h=" .. effective_h .. par .. ",pad=w=" .. effective_w .. ":h=" .. effective_h .. ":x=-1:y=-1,format=bgra"
  end
...
local function info(w, h)
  ...
  disabled =
    (w or 0) == 0 or (h or 0) == 0 or has_vid == 0 or (properties["demuxer-via-network"] and not options.network) or
  ...
  if pre_0_30_0 then
    ...
    mp.command_native_async(
      {"script-message", "thumbfast-info", json},
      function()
      end
    )
  end
...
local function clear()
  ...
  if pre_0_30_0 then
    ...
    mp.command_native_async(
      {"overlay-remove", options.overlay_id},
      function()
      end
    )
  end
...
local function watch_changes()
  ...
  local resized =
    old_w ~= effective_w or old_h ~= effective_h or last_vf_reset ~= vf_reset or (last_rotate % 180) ~= (rotate % 180) or
    par ~= last_par or
