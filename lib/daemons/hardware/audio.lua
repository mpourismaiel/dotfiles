...
function sink:set_volume(volume)
  volume = gears.math.round(volume)
    ...
    function(stdout)
      ...
      for line in stdout:gmatch("[^\r\n]+") do
        if line:match("Sink") or line:match("Source") then
          ...
          device.default = false
          gears.table.crush(device, device.type == "sinks" and sink or source, true)
...
local function get_applications(self)
  awful.spawn.easy_async_with_shell(
    ...
    function(stdout)
      local application = gears.object {}
      ...
      for line in stdout:gmatch("[^\r\n]+") do
        if line:match("Sink Input") or line:match("Source Output") then
          ...
          if new_application then
            application = gears.object {}
            application.id = id
            application.type = type
            gears.table.crush(application, application.type == "sink_inputs" and sink_input or source_output, true)
