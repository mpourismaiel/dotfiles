...
local function new()
  ...
  gears.table.crush(ret, upower, true)

  ret._private = {}

  ret._private.display_device_proxy =
  ...
  if ret._private.display_device_proxy then
    ret._private.display_device_proxy_properties =
      dbus_proxy.Proxy:new {
      bus = dbus_proxy.Bus.SYSTEM,
      name = "org.freedesktop.UPower",
    ...
    gears.table.crush(ret._private.display_device_proxy, device, true)

    if ret._private.display_device_proxy.IsPresent then
      ret._private.display_device_proxy_properties:connect_signal(
        "PropertiesChanged",
    ...
    gears.debug.print_warning(
      "Can't find UPower display device. " .. "Seems like UPower is not installed or your PC has no battery. "
    )
  end
