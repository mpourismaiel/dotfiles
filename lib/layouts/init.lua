...
function layout.inc(i, s, layouts)
	if type(i) == "table" then
		...
		gdebug.deprecate(
			"Use awful.layout.inc(increment, screen, layouts) instead" .. " of awful.layout.inc(layouts, increment, screen)",
			{deprecated_in = 5}
		)
			...
			if init_layouts then
				gdebug.print_warning(
					"`awful.layout.layouts` was set before `request::default_layouts` could " ..
						"be called. Please use `awful.layout.append_default_layouts` to " .. " avoid this problem"
				)
