...
function overflow:layout(context, orig_width, orig_height)
  ...
  for i, w in ipairs(widgets) do
    ...
    local content_w, content_h =
      base.fit_widget(self, context, w, need_scrollbar and width - self._private.scrollbar_spacing or width, height)
...
function overflow:scroll(amount)
  ...
  self:emit_signal("layout::scroll")
end
...
function overflow:set_scroll_factor(factor)
  ...
  if
    current == factor or -- the content takes less space than what is available, i.e. everything
      -- is already visible
      interval <= 0 or -- the scroll factor is out of range
...
local function apply_scrollbar_mouse_signal(container, w)
  w:connect_signal(
    "button::press",
    function(_, x, y, button_id, _, geo)
      if button_id ~= 1 then
        return
      end
...
local function new(dir, ...)
  ...
  ret._private.step = config.dpi(50)
  ret._private.fill_space = true
  ret._private.scrollbar_width = config.dpi(10)
  ret._private.scrollbar_enabled = true
  ret._private.scrollbar_position = dir == "vertical" and "right" or "bottom"
  ret._private.scrollbar_spacing = config.dpi(15)

  local scrollbar_widget = wscrollbar {}
