...
function object:dynamic_connect_signal(name, func)
  assert(type(func) == "function", "callback must be a function, got: " .. type(func))
  local sig = find_signal(self, name)
  assert(sig.strong[func] == nil, "Trying to connect a dynamic callback which is already connected strong")
  assert(sig.weak[func] == nil, "Trying to connect a dynamic callback which is already connected weakly")
  sig.dynamic[func] = make_the_gc_obey(func)
end
...
function object:dynamic_disconnect_signals(name)
  local sig = find_signal(self, name)
  for key, value in pairs(sig.dynamic) do
    sig.dynamic[key] = nil
    key = nil
  end
end
...
function object:emit_signal(name, ...)
  ...
  for func in pairs(sig.dynamic) do
    func(self, ...)
  end
...
local function set_miss(self, key, value)
  ...
  elseif (not rawget(self, "get_" .. key)) and not (class and class["get_" .. key]) then
    return rawset(self, key, value)
  else
    error("Cannot set '" .. tostring(key) .. "' on " .. tostring(self) .. " because it is read-only")
  end
end
