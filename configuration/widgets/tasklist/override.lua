...
local function default_template(self)
  local has_no_icon =
    self._private.style.disable_icon or self._private.style.tasklist_disable_icon or beautiful.tasklist_disable_icon
...
local function tasklist_update(s, self, buttons, filter, data, style, update_function, args)
  ...
  local clients = source and source(s, args) or capi.client.get()
...
function tasklist:set_base_layout(layout)
  self._private.base_layout = base.make_widget_from_value(layout or flex.horizontal)
...
function tasklist:set_screen(value)
  ...
  if value == self._private.screen then
    return
  end
...
function tasklist.new(args, filter, buttons, style, update_function, base_widget)
  ...
  if
   ...
   then
    gdebug.deprecate("The `screen` parameter is deprecated, use `args.screen`.", {deprecated_in = 5})
  ...
  for k, v in pairs {
    ...
    gdebug.deprecate(
      "The `awful.widget.tasklist()` `" .. k .. "` parameter is deprecated, use `args." .. k .. "`.",
      {deprecated_in = 5}
    )
    args[k] = v
  end
