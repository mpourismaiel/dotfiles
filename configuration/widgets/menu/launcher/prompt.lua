...
local function update(self)
    ...
    self.textbox:set_markup(
        prompt_text_with_cursor {
            text = self.command,
            text_color = self.fg_cursor,
            cursor_color = self.bg_cursor,
            cursor_pos = self._private_cur_pos,
            cursor_ul = self.ul_cursor,
...
function prompt:start()
    ...
    self.textbox:set_markup(
        prompt_text_with_cursor {
            text = self.reset_on_stop and self.text or self.command,
            text_color = self.fg_cursor,
            cursor_color = self.bg_cursor,
            cursor_pos = self._private_cur_pos,
            cursor_ul = self.ul_cursor,
    ...
    self._private.grabber =
        ...
        function(modifiers, key, event)
            ...
            if self.keypressed_callback then
                local user_catched, new_command, new_prompt = self.keypressed_callback(mod, key, self.command)
            ...
            if mod.Control then
                ...
                if key == "v" then
                    ...
                    if selection then
                        ...
                        self.command =
                            self.command:sub(1, self._private_cur_pos - 1) ..
                ...
                elseif key == "d" then
                    if self._private_cur_pos <= #self.command then
                        self.command =
                            self.command:sub(1, self._private_cur_pos - 1) ..
                            self.command:sub(self._private_cur_pos + 1)
                    end
                elseif key == "p" then
                ...
                elseif key == "r" then
                    self._private.search_term =
                        self._private.search_term or self.command:sub(1, self._private_cur_pos - 1)
                ...
                elseif key == "h" then
                    if self._private_cur_pos > 1 then
                        ...
                        self.command =
                            self.command:sub(1, self._private_cur_pos - 2 - offset) ..
            ...
            elseif mod.Mod1 or mod.Mod3 then
                ...
                if self.completion_callback then
                    if key == "Tab" or key == "ISO_Left_Tab" then
                        ...
                        local matches
                        self.command, self._private_cur_pos, matches =
                            self.completion_callback(command_before_comp, cur_pos_before_comp, ncomp)
                        ncomp = ncomp + 1
                        key = ""
                ...
                elseif key == "Delete" then
                    self.command =
                        self.command:sub(1, self._private_cur_pos - 1) .. self.command:sub(self._private_cur_pos + 1)
                ...
                elseif key == "Next" then
                    ...
                    if key:wlen() == 1 then
                        ...
                        self.command =
                            self.command:sub(1, self._private_cur_pos - 1) ..
                            key .. self.command:sub(self._private_cur_pos)
