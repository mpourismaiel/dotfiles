...
local function unselect_app(self)
  if self._private.active_widget ~= nil then
    ...
    if name_widget then
      name_widget.markup =
        string.format("<span foreground='%s'>%s</span>", self.app_name_normal_color, name_widget.text)
    end
    local generic_name_widget = self._private.active_widget:get_children_by_id("generic_name")[1]
    if generic_name_widget then
      generic_name_widget.markup =
...
local function create_app_widget(self, entry)
  ...
  local generic_name =
    entry.generic_name ~= nil and self.app_show_generic_name == true and
    {
      widget = wibox.widget.textbox,
      id = "generic_name",
      font = self.app_name_font,
  ...
  app:connect_signal(
    ...
    function(_self)
      ...
      if app.selected then
        ...
        local hover_color =
          (is_dark or is_opaque) and color.rgba_to_hex(color.multiply(app_normal_color, 2.5)) or
...
local function search(self, text)
  ...
  if text == "" then
    ...
    for index, entry in pairs(self._private.all_entries) do
      ...
      if
        string.find(entry.name:lower(), text:lower(), 1, true) ~= nil or
...
local function new(args)
  ...
  args.reverse_sort_alphabetically =
    args.reverse_sort_alphabetically ~= nil and args.reverse_sort_alphabetically or false
  ...
  args.hide_on_left_clicked_outside =
    args.hide_on_left_clicked_outside == nil and true or args.hide_on_left_clicked_outside
  args.hide_on_right_clicked_outside =
    args.hide_on_right_clicked_outside == nil and true or args.hide_on_right_clicked_outside
  ...
  args.app_selected_hover_color =
    args.app_selected_hover_color or
    (color.is_dark(args.app_normal_color) or color.is_opaque(args.app_normal_color)) and
  ...
  local grid_width =
    ret.shrink_width == false and
    dpi((ret.app_width * ret.apps_per_column) + ((ret.apps_per_column - 1) * ret.apps_spacing)) or
    nil
  local grid_height =
    ret.shrink_height == false and
    dpi((ret.app_height * ret.apps_per_row) + ((ret.apps_per_row - 1) * ret.apps_spacing)) or
    nil
